...
var TestFrameworkRunner = function () {
  ...
  var blockExecuted = function (block_counter, blockname, args) {
    ...
    if (blockseq[target] !== undefined
      ...
      if(checkArguments(blockseq[target], args)) {
        ...
        console.log("block of name " + blockname + " executed" + (args == undefined ? "" : ", argument: " + args.args));
    ...
    if(blockname != "wait:elapsed:from:") {
      console.log("Uninteresting block of name " + blockname + " executed" + (args == undefined ? "" : ", argument: " + args.args));
      console.log("Still expecting blocks " + JSON.stringify(blockseq));
    }
  ...
  var start = function () {
    ...
    if (typeof runtime === 'undefined' || !runtime.projectLoaded) {
      console.warn("1st condition 2 of NOT met, runtime.projectLoaded = false");
      console.info("Retrying in 100ms");
      if (!waiting_for_start) {
        setTimeout(function () { runner.start(); }, 100);
  ...
  var checkThen = function () {
    ...
    try {
      for (var i in testcase_spec['then']) {
        console.info(testcase_spec['then'][i][0] + " test ...");
