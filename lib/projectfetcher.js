...
var handleProjectJsonResponse = function(args) {
  ...
  if (status < 200 || 300 <= status)
    throw new Error("HTTP status code = " + status
      + " -- " + statuscodes.getStatusText(status));
  ...
  promises.push(
    Q.nfcall(fs.writeFile, path.join(projects_directory, projectId + '.json'), body)
...
var loadAsset = function(filename) {
  ...
  return httpGetPromise.promise
      .then(function(response){
        var output = fs.createWriteStream(path.join(projects_directory, filename));
