...
var TestFrameworkRunner = function () {
  ...
  var executeUserInteraction = function (sprite) {
    while (blockseq[sprite][0][0].substr(0, 4) === "user") {
      var input = blockseq[sprite].shift();
      ...
      switch (input[0]) {
        case 'userPointTo':
          msg.send(['pointTo', input[1], input[2]]);
          break;
        case 'userPointToSprite':
          var sprite = input[1];
          var costume = getVisibleCostume(sprite);
          var pos = getCostumePosition(sprite, costume);
          msg.send(['pointTo', pos[0], pos[1]]);
          break;
        case 'userClick':
          msg.send(['keypress', input[1]]);
          break;
        case 'userPress':
  ...
  var blockExecuted = function (block_counter, blockname, arguments) {
    ...
    if (blockseq[target] !== undefined
      ...
      if(checkArguments(blockseq[target], arguments)) {
        ...
        if (blockseq[target][0][0].substr(0, 4) === "user") {
          var interaction = blockseq[target].shift();
          msg.send(interaction);
        }
