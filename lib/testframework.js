...
var TestFrameworkRunner = function () {
  ...
  var checkArguments = function(blockname, arguments) {
    for(i = 1;i < blockname[0].length;i++) {
      if(blockname[0][i] != normalizeCostumeName(arguments.args[i - 1])) {
        console.log("Wrong Arguments: expected: -" + blockname[0][i] +
  ...
  var blockExecuted = function (blockname, arguments) {
    if(interp.activeThread.target != undefined) {
      ...
      console.log("target: " + interp.activeThread.target.objName);
      //console.log("JSON stringify: " + JSON.stringify(interp.activeThread.target));
    }
    
    if (blockseq.length > 0 && blockseq[0][0] === blockname) {
      if(checkArguments(blockseq, arguments)) {
        blockseq.shift();
